...
const CREATE_INDEX_MOVIE_RATINGS_TIME_CREATED = `CREATE INDEX movieRatings_time_created_idx ON movie_ratings (time_created);`
...
describe("Tables", () => {
  ...
  it("should have indices", async done => {
    ...
    expect(movieRatings).toEqual([
      { 
        name: "movieRatings_time_created_idx",
        unique: false 
      },
      { 
        name: "sqlite_autoindex_movie_ratings_1",
        unique: true 
      }
    ]);
